...
export const GetEntranceFee = () => {
  const { data: entranceFee, refetch: refetchEntranceFee } =
    useLotteryGetEntranceFee({
      address,
    });

  const { data: numberOfPlayers, refetch: refetchNumberOfPlayers } =
    useLotteryGetNumberOfPlayers({
      address,
    });

  const { data: recentWinner, refetch: refetchRecentWinner } =
    useLotteryGetRecentWinner({ address });
